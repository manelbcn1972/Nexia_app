...
// Scheduled via netlify.toml: runs daily at 17:00 UTC (18:00 CET)
...
export async function handler(event) {
  try {
    ...
    if (!SUPABASE_URL || !SUPABASE_KEY || !RESEND_KEY) {
      console.error('Missing credentials:', { url: !!SUPABASE_URL, key: !!SUPABASE_KEY, resend: !!RESEND_KEY })
    ...
    const entriesRes = await fetch(
      `${SUPABASE_URL}/rest/v1/daily_entries?select=patient_id&created_at=gte.${today}T00:00:00Z`,
      {
        headers: {
          'apikey': SUPABASE_KEY,
          'Authorization': `Bearer ${SUPABASE_KEY}`
    ...
    const checkedInIds = new Set(Array.isArray(todayEntries) ? todayEntries.map(e => e.patient_id) : [])
    ...
    for (const patient of missing) {
      ...
      try {
        ...
        console.log(`Reminder to ${patient.name} (${patient.email}):`, JSON.stringify(result))
        sent++
      } catch (err) {
    ...
    // Notify therapist(s) about missing check-ins
    ...
    const therapistEmails = [...new Set([...(Array.isArray(therapists) ? therapists.map(t => t.email) : []), ...ccEmails].filter(Boolean))]
    ...
    if (therapistEmails.length > 0) {
      ...
      try {
        ...
        console.error('Failed to notify therapists:', err.message)
      }
    }
    ...
    console.error('Reminder function error:', err.message, err.stack)
